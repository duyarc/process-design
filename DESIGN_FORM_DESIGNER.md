...
| 2026-08-27 | **Executive Editorial Pair 1 H1-H2 Typography & Spacing Standardization:** (1) Standardized **H1** across Canvas, Viewers, and Print out to use Gestalt proximity with pure bold uppercase typography (`fontSize: 1.1rem`, `fontWeight: 700`, `letterSpacing: 0.6px`, `color: #0f172a`, `border: 'none'`, `background: 'transparent'`), eliminating divisive horizontal underlines that fragmented the form. (2) Standardized **H2** to use a sleek Left Accent Bar (`borderLeft: '3px solid var(--primary)'`, `padding: '2px 0 2px 8px'`) with transparent background (`background: 'transparent'`, `fontSize: 0.92rem`–`0.95rem`), eliminating gray banner boxes for optimal visual flow. (3) Uniformly applied across `FormBuilder.tsx`, `ReportBuilder.tsx`, `FormFiller.tsx`, `ProcessReader.tsx`, `PrintBlankForm.tsx`, `PrintFilledForm.tsx`, `PrintRecord.tsx`, `FormReport.tsx`, and `PrintReport.tsx`. |
