...
| 2026-08-28 | **Direct In-Cell Option Label Editing & 1-Click Default Reset Icon:** (1) Enabled seamless in-place label editing on Canvas table Radio/Checkbox cells via auto-grow `<textarea>` overlay with `handleFormatKeyDown` (`Ctrl+B/I/U`), preserving clean horizontal 1-line alignment (`○ Có ○ Không`). (2) Added subtle top-right `🔄` 1-click reset button on customized cells to instantly revert to column defaults. (3) Cleaned up Right Inspector to focus 100% on Column and Section settings. |
