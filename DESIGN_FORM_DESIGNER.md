...
| **Verified At Commit** | (2026-09-11) — Table Floating Hover Overlay & Line Count UI/UX refinement verified against source |
...
| 2026-09-11 | **Table Floating Hover Overlay & Line Count UI/UX Refinement:** (1) Completely eliminated 54px action column from table `<colgroup>`, `<thead>`, `<tbody>` for true WYSIWYG parity. (2) Relocated `GripVertical` drag handle to a floating pill at row start (`left: 3px`). (3) Upgraded row action pill (`right: 4px`): replaced `{lc}↕` text glyph with vector icon `Rows2` + count + `ChevronDown` in a borderless hover button, upgraded group header switch to `RotateCcw`, and added subtle dividers and hover polish. |
