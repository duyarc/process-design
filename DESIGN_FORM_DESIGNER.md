...
| 2026-08-31 | **In-Canvas Table Top Bar Controls (Border Style & Header Toggle):** (1) Extended `InCanvasTitleHeader` with `extraControls` support. (2) Integrated 3-button Border style pill `[ ⊞ (Grid) | ☰ (Horizontal) | ⬚ (Borderless) ]` directly on the Top Bar next to `[ H1 | H2 | Body | None ]`. (3) Added zero-text-label Header toggle button using `PanelTop` icon (active primary when visible, light gray when hidden) with instantaneous 2-way sync with Right Inspector. |
