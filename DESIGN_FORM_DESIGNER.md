...
| 2026-08-28 | **Table In-Cell Formatting Shortcuts & Radio/Checkbox Option Label Rich Rendering:** (1) Enabled `handleFormatKeyDown` (`Ctrl+B`, `Ctrl+I`, `Ctrl+U`) on Canvas Table In-Cell `<textarea>` in `FormBuilder.tsx`. (2) Wrapped `renderFormattedText(opt.label)` for table Radio and Checkbox option labels across `FormFiller.tsx`, `ProcessReader.tsx`, `PrintBlankForm.tsx`, `PrintFilledForm.tsx`, and `PrintRecord.tsx`. |
