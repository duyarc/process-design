...
| 2026-08-14 | `CURRENT` | **Table Cell AcroField Width Truncation & Y-Axis Vertical Centering:** Updated `acroFormOverlay.ts` to deduct 16pt from measured outer cell width (`rawWidth = cellWidthPt - 16`) and calculate exact Y-axis vertical centering (`fieldY = pdfPageHeight - marginY - (localTop * scaleFactor + (cellHeightPt + fieldHeight) / 2)`). Completely eliminates right border line overlap and centers AcroFields vertically inside table cells. |
| 2026-08-14 | `CURRENT` | **Dedicated Inner Span Placeholder Architecture for Dynamic Table Columns:** Updated `PrintBlankForm.tsx` and `acroFormOverlay.ts`. Replaced `<td>` level acrofield flags with explicit inner `<span data-acroform-field="true">` elements embedded inside each table cell, enabling 1:1 precise DOM scanning and alignment for Column 1 (`Tên sản phẩm`), Column 2 (`Quy cách`), Column 3 (`Số lượng`), etc. |
