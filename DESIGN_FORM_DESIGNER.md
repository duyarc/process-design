...
| 2026-08-28 | **Direct In-Cell Option Management (Add, Edit, Delete & Clean Reset):** (1) Integrated clean, borderless `RotateCcw` icon in top-right cell corner to restore column defaults without orange border/background. (2) Added mini `Plus` icon in top-right cell corner to instantly add new options in-cell. (3) Added in-place `✕` delete button beside each option label (guarded by `length > 1`). (4) Preserved clean auto-grow `<textarea>` inline layout with format shortcuts (`Ctrl+B/I/U`). |
