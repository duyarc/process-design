...
| 2026-08-28 | **Table Option Cell WYSIWYG Parity & Top-Level Cell Inspector Card:** (1) Replaced cluttered in-cell `<textarea>` + delete `✕` + `+ Thêm lựa chọn` buttons across all table cells in `FormBuilder.tsx` with clean 100% WYSIWYG preview (`○ Có ○ Không` side-by-side or stacked), achieving exact visual parity with `FormFiller` and Print views. (2) Relocated "✨ Tùy biến Lựa chọn cho Ô này" card from below the fold to the very top of the Table Properties Inspector (above `Cấu hình Cột`), ensuring instant visibility without scrolling upon clicking any Option Cell. |
