...
| 2026-08-28 | **Table Option Cell WYSIWYG Parity & In-Cell De-cluttering:** (1) Replaced cluttered in-cell `<textarea>` + delete `✕` + `+ Thêm lựa chọn` buttons across all table cells in `FormBuilder.tsx` with clean 100% WYSIWYG preview (`○ Có ○ Không` side-by-side or stacked), achieving exact visual parity with `FormFiller` and Print views. (2) Maintained seamless cell option customization via Right Inspector (`activeCellKey` card). |
