...
| 2026-08-28 | **Group Header Row Typography & Contrast Standardization:** (1) Upgraded `<strong>` weight in `textFormatter.tsx` from `600` to `700` (`var(--pw-weight-heavy)`), delivering powerful contrast between bold titles and body text. (2) Standardized Group Header base weight from hardcoded `600` to regular `400` (`var(--pw-weight-regular)`) and font size to `0.82rem` (`--pw-font-body`) with `lineHeight: 1.45` across `FormBuilder.tsx`, `FormFiller.tsx`, `ProcessReader.tsx`, `PrintBlankForm.tsx`, `PrintFilledForm.tsx`, and `PrintRecord.tsx`. |
