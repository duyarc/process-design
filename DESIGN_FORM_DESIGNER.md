...
| **Verified At Commit** | `CURRENT` (2026-08-25) — Sections 2 & 3 (Contextual Block Insertion After Active Block: FormBuilder.tsx handleAddBlock & handleExecuteCopy). |
...
| 2026-08-25 | `CURRENT` | **Contextual Block Insertion (Insert After Active Block):** Updated `handleAddBlock` and `handleExecuteCopy` in `FormBuilder.tsx`. When a block is currently selected (`activeBlockId`), newly added or copied blocks are inserted at `activeIdx + 1` (immediately following the active block), rather than unconditionally appending to the end of the canvas. |
