...
| 2026-08-28 | **Group Header Row Rich Formatting & Inspector Toolbar:** (1) Added `inspectorGroupTitleRef` and `[ B ] [ I ] [ U ]` formatting toolbar buttons in Right Inspector Panel for active group header rows in `FormBuilder.tsx`. (2) Integrated `handleFormatKeyDown` (`Ctrl+B`, `Ctrl+I`, `Ctrl+U`) on both Canvas group header textarea and Right Inspector textarea, enabling instant rich-text inline formatting for table group header titles. |
