...
| 2026-08-28 | **Form Properties 1-Line 2-Toggle Layout & Public Link Management:** (1) Added `isPublic?: boolean` (default `false`) and `defaultFocusMode?: boolean` (default `false`) to `FormTemplateISO` in `types.ts` and backend `server.cjs`. (2) In `FormBuilder.tsx` Right Inspector (Properties tab), combined `Focus mode` and `Public link` switch toggles into a balanced 1-line horizontal row. (3) Added direct URL input group with 1-click Copy button immediately below the toggle row, automatically providing the private link (`/?form=...`) or public short link (`/f/:slug`) with instant feedback. |
