...
| **Verified At Commit** | (2026-08-27) — Section 2, 4 (Section Focus & Accordion Mode for Long Forms in FormFiller). |
...
| 2026-08-27 | `CURRENT` | **Section Focus & Accordion Mode for Long Forms (5S/5C Scorecard Parity):** (1) Built `groupBlocksIntoSections` in `formUtils.ts` partitioning `layoutBlocks` into chapters based on `SECTION_LABEL` / H1 boundaries with smart deduplication for leading blocks (100% zero-breaking change with legacy templates). (2) Built `computeSectionProgress` calculating real-time filled field ratios across Info Grid, Tables, Likert scales, Matrix tables, and Sign-offs. (3) Streamlined Top Navigation Bar to a single balanced row with `[ 🎯 Focus mode (──🟢) ]` switch toggle, `[ 🖨 In bản khai ]`, and split share badge; removed redundant `PDF`, `In form trắng`, and horizontal stepper scrollbar. (4) Implemented Accordion Focus cards with section-level footer navigation (`[ ← Quay lại ]` / `[ Tiếp tục: ... → ]` / `[ 🚀 Gửi phiếu ]`) and smooth scroll transitions. |
