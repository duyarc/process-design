...
| 2026-08-28 | `CURRENT` | **Nested Collapsible H2 Sub-Accordions in Focus Mode:** (1) Upgraded `FormSectionGroup` in `formUtils.ts` with `leadingBlocks` and `subSections: FormSubSectionGroup[]` (extracting H2 sub-sections within each H1 parent). (2) Built interactive Single-Active H2 Sub-Accordion in `FormFiller.tsx`: When H1 opens, H2 #1 opens by default; clicking another H2 expands it and closes previous H2; clicking active H2 collapses it. H2 headers feature Left Accent Bar (`borderLeft: 3px solid var(--primary)`) and mini `[ + ]` / `[ − ]` indicator with subtle left border guide (`borderLeft: 2px solid rgba(13, 148, 136, 0.15)`). |
