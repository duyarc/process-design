...
| 2026-08-26 | `CURRENT` | **100% Designed Table Rows Preservation & Strict Auto-Append:** Updated `initSmartTableRows` in `FormFiller.tsx` to return 100% of template rows defined in Form Designer without collapsing empty rows or injecting extra rows on load. Retained manual deletion `🗑️` and Auto-Append row generation strictly when typing into the last row. |
