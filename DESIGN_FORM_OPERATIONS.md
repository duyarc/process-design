...
| 2026-08-26 | `CURRENT` | **Dynamic Table Rows & Smart Auto-Clean Empty Rows on Load:** Integrated `tableRowsMap` & `initSmartTableRows` in `FormFiller.tsx`. Preserves fixed static label rows (`static_text`), automatically collapses free empty input rows down to 1 initial blank row on load, and provides minimal `[ ✚ Thêm dòng ]` button + `🗑️` delete icon row actions. |
