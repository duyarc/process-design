...
| 2026-08-27 | `CURRENT` | **Section Focus & Accordion Mode for Long Forms (5S/5C Scorecard Parity):** (1) Built `groupBlocksIntoSections` in `formUtils.ts` partitioning `layoutBlocks` into chapters based on `SECTION_LABEL` / H1 boundaries (100% zero-breaking change with legacy templates). (2) Built `computeSectionProgress` calculating real-time filled field ratios across Info Grid, Tables, Likert scales, Matrix tables, and Sign-offs. (3) Added Sticky Stepper Tracker with dynamic status pills (`[✓ 5/5]` / `[● 3/5]` / `[○ 0/5]`) and segmented View Mode toggle (`[ 📑 Từng phần (Focus) | 📄 Toàn bộ (All) ]`). (4) Implemented Accordion Focus cards with section-level footer navigation (`[ ← Quay lại ]` / `[ Tiếp tục: ... → ]` / `[ 🚀 Gửi phiếu ]`) and smooth scroll transitions. |
