...
| 2026-08-27 | `CURRENT` | **Seamless H1 Section Focus & Accordion Mode:** (1) Built `groupBlocksIntoSections` in `formUtils.ts` with strict H1-only grouping (excluding H2, keeping H2 subheaders inside their parent H1 section). (2) Seamless H1 Accordion Header in `FormFiller.tsx`: H1 title retains 100% exact ISO typography with subtle `[ + ]` (when collapsed) and `[ − ]` (when expanded) toggle indicators; clicking H1 row smoothly expands/collapses child blocks beneath it without shifting layout. (3) Eliminated noisy progress badges `[ 5/5 ]`, counter circles, and in-section navigation footer (`[ ← Quay lại ]` / `[ Tiếp tục → ]`). (4) Top Navigation Bar streamlined to 1 balanced row with `[ 🎯 Focus mode (──🟢) ]` toggle switch. |
