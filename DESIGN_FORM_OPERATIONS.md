...
| 2026-08-26 | `CURRENT` | **Unified Grouped Dynamic Table Rows & Per-Group Add Action:** Upgraded `initSmartTableRows` and `handleAddTableRowToGroup` in `FormFiller.tsx`. Preserves 1 initial blank row under each group header (`isGroupHeader`), provides a unified `[ ✚ Thêm dòng ]` button at the bottom of each group section, and keeps ungrouped tables consistent with a single bottom `[ ✚ Thêm dòng ]` button. |
