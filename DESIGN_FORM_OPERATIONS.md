...
| 2026-08-26 | `CURRENT` | **Auto-Append Dynamic Table Rows (Excel / Google Sheets Pattern):** Implemented `handleTableCellChangeWithAutoAppend` in `FormFiller.tsx`. Completely eliminated all `+ Thêm dòng` buttons to achieve 100% clean, buttonless dynamic tables that auto-spawn new blank rows when typing into the last cell of any group/table. |
