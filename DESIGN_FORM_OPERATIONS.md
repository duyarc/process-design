...
| **Verified At Commit** | `CURRENT` (2026-08-17) — Section 3 (PrintFilledForm group-level multi-tbody page breaking matching PrintBlankForm). |
...
| 2026-08-17 | `CURRENT` | **PrintFilledForm Multi-`<tbody>` Group Page Breaking:** Updated `PrintFilledForm.tsx` to group rows into `.print-table-group` (`<tbody>` with `page-break-inside: avoid`), allowing filled forms to break cleanly across pages by group unit. |
