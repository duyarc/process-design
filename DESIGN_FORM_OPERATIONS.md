...
| 2026-08-26 | `CURRENT` | **Blank Screen Hotfix & FormErrorBoundary Protection:** Fixed uncaught `TypeError` on `block.fields` for `PAGE_BREAK` / `SECTION_LABEL` blocks in `FormFiller.tsx` (safeguarded with `(block.fields || [])`). Wrapped `FormFiller` in a `FormErrorBoundary` to prevent white screens and show graceful error recovery cards. |
