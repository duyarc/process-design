...
| 2026-08-28 | `CURRENT` | **Form Experience & Public Link Centralization in FormBuilder:** (1) Centralized `isPublic` and `defaultFocusMode` into FormBuilder template properties (defaulting both to `false`). (2) Streamlined `FormFiller` Top Navigation Bar to a clean row with text `"Focus mode"` (emoji removed), `[ 🖨 In bản khai ]`, and a single unified `[ 🔗 Sao chép link ]` button without admin toggles. (3) Replaced `alert()` with non-blocking toast notifications (`setLocalToast`) for copy feedback. |
