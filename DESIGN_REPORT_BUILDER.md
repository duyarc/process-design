...
| **Verified At Commit** | (2026-09-24) — Base-5 (`Thang 5`) Default Score Mechanism (`reportScoring.ts`, `reportCompute.ts`, `FieldScoringInspector.tsx`) & `SmartNumberInput` (Auto-Select on Focus + Focus-Only Quick-Select Pill Bar on `Weight %`) verified against source |
