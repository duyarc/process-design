...
| **Verified At Commit** | (2026-09-24) — Streamlined Left Sidebar Option 3 Icon-First (`SOURCE` with formTitle only, `SUBMISSION`, default-hidden `FIELDS` bar with icon controls, `40px` collapsed rail) & `INFO_GRID` `locationCode` grouping verified against source |
...
| Date | Change |
|---|---|
| 2026-09-24 | **Streamlined Left Sidebar Option 3 (Icon-First & Clean Labels) & `INFO_GRID` Element Grouping:** (1) In `ReportBuilder.tsx`, renamed `1. SOURCE FORM` to `SOURCE` (displaying only `f.formTitle || f.formId` without redundant ID prefix) and `2. SAMPLE SUBMISSION` to `SUBMISSION`. (2) Default-collapsed the `FIELDS` tree (`isFieldsTrayOpen = false`) behind a compact icon bar (`Layers` + `FIELDS` + count badge + `ChevronRight` / `ChevronDown`, plus `ChevronsUpDown` / `ChevronsDownUp` icon buttons for Expand/Collapse All when open). (3) Added `isLeftSidebarCollapsed` (`PanelLeftClose` / `PanelLeftOpen`) to fold the Left Sidebar into a `40px` icon rail. (4) In `tableFieldExtractor.ts`, normalized `extractAllFormFields` `locationCode` for `INFO_GRID` fields to `elementName || f.locationCode` so fields group cleanly under their `INFO_GRID` block title rather than individual `LOC-*` codes. |
