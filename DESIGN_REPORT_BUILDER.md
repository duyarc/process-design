...
| **Verified At Commit** | (2026-09-24) — 2-Step Design Principle (Build Layout First -> Fill Content Into Layout) in FormReferenceCanvas & complete interactive TITLE block in ReportBuilder verified against source |
...
| Date | Change |
|---|---|
| 2026-09-24 | **2-Step Design Principle (Build Layout First → Fill Content Into Layout) & Complete Title Block:** (1) In `FormReferenceCanvas.tsx`, implemented `renderLayoutBlockShell` enforcing the 2-step FormBuilder architecture: Step 1 builds the standardized Layout Block Container (`border: 1px dashed #cbd5e1` idle, `2px solid var(--primary)` active, top-right floating `{block.type}` badge, `isSeamlessTableBlock` border handling) and complete `TITLE` block structural slots (Logo, `<h1>` Title, `<p>` Subtitle defaulting to `(mô tả ngắn kiểm tra)`, and Date `Ngày ___/___/____`); Step 2 populates data pulled from `form.layoutBlocks` merged with `reportBlocks` overrides. (2) In `ReportBuilder.tsx`, added `syncTitleBlockFromForm` in `init()` and `handleFormChange()` to auto-initialize and sync the `TITLE` block from the source form (`"5C SCORECARD"`), added `handleSelectBlockFromFormCanvas` mapping `FormReferenceCanvas` block clicks (`TITLE`, `SECTION_LABEL`, `INFO_GRID`, `SIGN`, `TABLE`) to `template.layoutBlocks`, standardized the Report tab block shell and `TITLE` block slots, and upgraded `Title Block Properties` in Right Inspector with `[TITLE]` badge and Seamless Borderless Title Input (`✎`). |
