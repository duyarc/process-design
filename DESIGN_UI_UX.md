...
| 2026-08-27 | `CURRENT` | **Long Form Section Focus & Sticky Stepper UI Pattern:** Added Section Focus & Accordion Mode design pattern to FormFiller. Features frosted glass sticky stepper tracker (`backdrop-filter: blur(12px)`), state-driven progress pills (`[✓]`, `[●]`, `[○]`), Collapsed Chapter Header Cards with counter badges, and section footer navigation (`[← Quay lại]`, `[Tiếp tục →]`). |
